...
#if defined(CONFIG_LEON2)
#define UART1_IRQ 3
#define UART2_IRQ 2
#elif defined(CONFIG_LEON3)||defined(CONFIG_LEON4)
#define UART1_IRQ 2
#define UART2_IRQ 3
...
static inline void _WriteUart(xm_u32_t base, xm_s32_t a) {
    ...
    while(!(LoadIoReg(base+STTS_REG)&STTS_TH_B)&&(loop<CONFIG_UART_TIMEOUT)) loop++;
...
#ifdef CONFIG_LEON2
    StoreIoReg(IODIR_REG+LEON_GPIO_BASE, (LoadIoReg(IODIR_REG+LEON_GPIO_BASE) & MASK_PIN_INPUT_DEFAULT_UART)| MASK_PIN_OUTPUT_DEFAULT_UART);
#endif
