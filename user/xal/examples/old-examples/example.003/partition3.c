...
#define PORT3_NAME "sport3_w"
#define PORT_SIZE 12
#define REFRESH_PERIOD 1000000L

static int port1, port3;
int niter;
...
void TrapHandler(trapCtxt_t *ctxt) {
    ...
    value2 = value * 3;
    w=XM_write_sampling_message(port3, &value2, sizeof(long));
...
void PartitionMain(void) {
    ...
    if ((port3=XM_create_sampling_port(PORT3_NAME, PORT_SIZE, XM_SOURCE_PORT, REFRESH_PERIOD))<0) {
        printf("[P%d] port [%s] cannot be created\n", XM_PARTITION_SELF, PORT3_NAME);
        return;
    }
