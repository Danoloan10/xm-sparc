...
static struct xmc *ParseXmc(struct file2Proc *xmcFile) {
    ...
    if (RWORD(xmcFile->xef.hdr->flags)&XEF_COMPRESSED) {
        ...
        for (ptr=0, e=0; e<RWORD(xmcFile->xef.hdr->noSegments); e++, ptr+=RWORD(xmcFile->xef.segmentTab[e].fileSize)) {
            ...
            Uncompress(RWORD(xmcFile->xef.segmentTab[e].deflatedFileSize), RWORD(xmcFile->xef.segmentTab[e].fileSize), URead, &rPtr, UWrite, &wPtr, 0);
        }
...
static int RId2Id(int partId, struct xmc *xmcTab) {
    struct xmcPartition *xmcPartitionTab;
    int n;
    xmcPartitionTab=(struct xmcPartition *)&((char *)xmcTab)[RWORD(xmcTab->partitionTabOffset)];
    for (n=0; n<RWORD(xmcTab->noPartitions); n++) {
        if (RWORD(xmcPartitionTab[n].rId)==partId)
            return RWORD(xmcPartitionTab[n].id);
    }
    return RWORD(xmcTab->noPartitions);
}
...
void DoCheck(int argc, char **argv) {
    ...
    if ((ret=XEF_parse_file(LoadFile(xmc.name), &xmc.xef, &xefOps))!=XEF_OK)
    ...
    for (e=2; e<argc; e++) {
        if (!strcmp(argv[e], "-h")) {
            ...
            partId=RId2Id(partId, xmcTab);
        } else {
            fprintf(stderr, "Ignoring unexpected argument (%s)\n", argv[e]);
            continue;
        }
        if ((ret=XEF_parse_file(LoadFile(part.name), &part.xef, &xefOps))!=XEF_OK)
            EPrintF("Error loading XEF file \"%s\": %d", part.name, ret);
        if (noCustom) {
            for (i=0; i<noCustom; i++) {
                if ((ret=XEF_parse_file(LoadFile(customTab[i].name), &customTab[i].xef, &xefOps))!=XEF_OK)
