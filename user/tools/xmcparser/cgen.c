...
/*
  ...
  - [30/03/2016:CP-250216-01]  Ángel Esquinas (aesquinas@fentiss.com)
    added watchdog node to hypervisor
*/
...
static inline void GenerateHpvHm(FILE *oFile, struct xmcHmSlot *hmTab) {
    extern char *hmActions[];
    extern char *hmEvents[];
    extern char *hmLog[];
    int i;
    fprintf(oFile, ADDNTAB(1, ".hmTab = {\n"));
    for (i=0; i<XM_HM_MAX_GENERIC_EVENTS+XM_HM_MAX_ARCH_EVENTS; i++)
        if (hmEvents[i])
...
static inline void GeneratePartitionHm(FILE *oFile, struct xmcHmSlot *hmTab) {
    extern char *hmActions[];
    extern char *hmEvents[];
    extern char *hmLog[];
    int i; 
...
#ifdef CONFIG_PLAN_EXTSYNC
                xmcSchedCyclicPlanTab[j].extSync, xmcSchedCyclicPlanTab[j].interval,
#endif
...
static inline void GenerateHpv(FILE *oFile) {
    ...
    fprintf(oFile,
	    ADDNTAB(2, "},\n"));
    GenerateHpvHm(oFile, xmc.hpv.hmTab);
    fprintf(oFile,
	    ADDNTAB(2, ".hwIrqTab = {\n"));
    ...
    fprintf(oFile, ADDNTAB(2, ".watchdog = {\n"));
    fprintf(oFile, ADDNTAB(3, ".wdInitialValue = %u,\n"), xmc.hpv.watchdog.wdInitialValue);
    fprintf(oFile, ADDNTAB(3, ".hwWdReloadValue = %u,\n"), xmc.hpv.watchdog.hwWdReloadValue);
    fprintf(oFile, ADDNTAB(2, "},\n"));
...
static inline void GeneratePartitionTab(FILE *oFile) {
	...
	fprintf(oFile,
		...
		ADDNTAB(2, ".id = %d,\n")
                ADDNTAB(2, ".rId = %d,\n")
		ADDNTAB(2, ".nameOffset = %d,\n")
                ADDNTAB(2, ".entryPoint = 0x%x,\n")
		ADDNTAB(2, ".flags = 0x%x,\n"),
		e, xmcPartitionTab[e].id, xmcPartitionTab[e].rId, xmcPartitionTab[e].nameOffset, xmcPartitionTab[e].entryPoint, xmcPartitionTab[e].flags);
	...
	GeneratePartitionHm(oFile, xmcPartitionTab[e].hmTab);
	fprintf(oFile,
		ADDNTAB(2, ".noPorts = %d,\n")
...
static void GenerateRsvMem(FILE *oFile) {
    ...
    for (i=0; i<xmc.noPartitions; i++) {
        ...
        RsvBlock(CONFIG_NO_VCPUS*pctSize, PAGE_SIZE, "partition control tables");
    }
