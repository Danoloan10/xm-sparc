...
#define USAGE "usage: "TOOL_NAME" [-c] [-d] [-e] [-s <xsd_file>] [-o output_file] <XM_CF.xml>\n"

char *inFile;

void LineWarning(int nLine, char *fmt, ...) {
    va_list args;
...
int main(int argc, char **argv)  {
    ...
    FILE *outFile=0;
    int opt, onlyCCode=0, printXmcSize=0;

    LIBXML_TEST_VERSION;

    while ((opt=getopt(argc, argv, "do:s:cC:e"))!=-1) {
        switch (opt) {
        case 'c':
            onlyCCode=1;
            break;
    ...
    if (printXmcSize) { 
        fprintf(stderr, "XM_CF size: %d\n", FindXmcSize(outFileName));
        exit(0);
    }
