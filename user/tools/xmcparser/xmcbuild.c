...
#define CMD "cat %s | ${XTRATUM_PATH}/scripts/preprocess.py > /tmp/tmp01.cfg; . /tmp/tmp01.cfg; rm /tmp/tmp01.cfg;  ${TARGET_CC} ${TARGET_CFLAGS_ARCH} -x c %s --include xm_inc/config.h --include xm_inc/arch/arch_types.h %s -o %s -Wl,--entry=0x0,-T%s\n"
#define CFLAGS "-O2 -Wall -I${XTRATUM_PATH}/user/libxm/include -I${XTRATUM_PATH}/include -nostdlib -nostdinc"
...
xmSize_t FindXmcSize(char *out) {
    ...
    lseek(fd, offsetof(struct xmc, size), SEEK_SET);

    DO_READ(fd, &xmcSize, sizeof(xmSize_t));
    xmcSize=RWORD(xmcSize);
