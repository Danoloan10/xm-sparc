...
static xm_s32_t CRead(void *obj, void *b, xmSize_t s, void *d) {
    memcpy(b, *(xm_u8_t **)d, s);
    *(xm_u8_t **)d+=s;
    return s;
}

static xm_s32_t CWrite(void *obj, void *b, xmSize_t s, void *d) {
...
static void CompressImage(void) {
	...
	if ((xefSegmentTab[e].deflatedFileSize=Compress(xefSegmentTab[e].fileSize, size, CRead, &ptrImg, CWrite, &ptrCImg, CSeek, 0))<=0)
