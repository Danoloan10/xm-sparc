...
static inline void XM_sparc_ctrl_winflow(void) {
    __asm__ __volatile__("mov "TO_STR(sparc_ctrl_winflow_nr)", %%o0\n\t" \
                         __DO_XMAHC:::"o0");
}

extern xm_s32_t XM_sparc_inport(xm_u32_t port, xm_u32_t * value);
extern xm_s32_t XM_sparc_outport(xm_u32_t port, xm_u32_t value);
extern xm_s32_t XM_sparc_outport_sdp(xm_u8_t *dst, xm_u8_t *src, xm_u32_t size);
extern xm_s32_t XM_sparc_disable_sdp(xm_u32_t chipSelect);
#ifdef CONFIG_SPW_RTC
extern xm_s32_t XM_sparc_outport_msg(xm_u32_t port, struct ioOutPortMsg * ioMsg, xm_u32_t timeout);
#endif
