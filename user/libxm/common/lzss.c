...
static inline xm_s32_t ReadByte(xm_u32_t *readCnt, xm_u32_t inSize, CFunc_t Read, void *rData, void *obj) {
    xm_u8_t tmp = 0;
    if (*readCnt>=inSize) return -1;
    if (Read(obj, &tmp, 1, rData)!=1) return -1;
    (*readCnt)++;
    return tmp&0xff;
}

static inline void WriteByte(xm_s32_t c, xm_u32_t *writeCnt, xm_u32_t outSize, CFunc_t Write, void *wData, void *obj) {
    xm_u8_t tmp;
    if (*writeCnt>=outSize) return;
    tmp=c;
    if (Write(obj, &tmp, 1, wData)!=1) return;
    (*writeCnt)++;
}
...
xm_s32_t LZSSCompress(xm_u32_t inSize, xm_u32_t outSize, CFunc_t Read, void *rData, CFunc_t Write, void *wData, void *obj) {
    ...
    for (len = 0; len < F && (c = ReadByte(&readCnt, inSize, Read, rData, obj)) != -1; len++)
	...
	if ((mask <<= 1) == 0) {  /* Shift mask left one bit. */
		...
		WriteByte(codeBuf[i], &writeCnt, outSize, Write, wData, obj);     /* code together */
	    codeBuf[0] = 0;  codeBufPtr = mask = 1;
	}
	lastMatchLength = matchLength;
	for (i = 0; i < lastMatchLength &&
		 (c = ReadByte(&readCnt, inSize, Read, rData, obj)) != -1; i++) {
	...
	for (i = 0; i < codeBufPtr; i++) WriteByte(codeBuf[i], &writeCnt, outSize, Write, wData, obj);
    }
    return writeCnt;
}
#endif

xm_s32_t LZSSUncompress(xm_u32_t inSize, xm_u32_t outSize, CFunc_t Read, void *rData, CFunc_t Write, void *wData, void *obj) {
    xm_s32_t  i, j, k, r, c;
    xm_u32_t  flags;
    xm_u32_t readCnt=0, writeCnt=0;
    ...
    for ( ; ; ) {
	if (((flags >>= 1) & 256) == 0) {
	    if ((c = ReadByte(&readCnt, inSize, Read, rData, obj)) == -1) break;
	    flags = c | 0xff00;          /* uses higher byte cleverly */
	}                                                 /* to count eight */
	if (flags & 1) {
	    if ((c = ReadByte(&readCnt, inSize, Read, rData, obj)) == -1) break;
	    WriteByte(c, &writeCnt, outSize, Write, wData, obj); textBuf[r++] = c;  r &= (N - 1);
	} else {
	    if ((i = ReadByte(&readCnt, inSize, Read, rData, obj)) == -1) break;
	    if ((j = ReadByte(&readCnt, inSize, Read, rData, obj)) == -1) break;
...
#ifndef _XM_KERNEL_
xm_s32_t Compress(xm_u32_t inSize, xm_u32_t outSize, CFunc_t Read, void *rData, CFunc_t Write, void *wData, void (*SeekW)(xmSSize_t offset, void *wData), void *obj) {
    ...
    if ((lzSize=LZSSCompress(inSize, outSize-sizeof(struct compressHdr), Read, rData, Write, wData, obj))<=0)
    ...
    if (Write(obj, &hdr, sizeof(struct compressHdr), wData)!=sizeof(struct compressHdr))
        return COMPRESS_WRITE_ERROR;
    return (lzSize+sizeof(struct compressHdr));
}
#endif

xm_s32_t Uncompress(xm_u32_t inSize, xm_u32_t outSize, CFunc_t Read, void *rData, CFunc_t Write, void *wData, void *obj) {
    struct compressHdr hdr;
    xm_u32_t lzSize = 0, size = 0, uncomSize = 0;
    hdr.magic = 0;
    hdr.size = 0;
    hdr.lzSize = 0;
    if (Read(obj, &hdr, sizeof(struct compressHdr), rData)!=sizeof(struct compressHdr))
    ...
    if (outSize<size)
	return COMPRESS_BUFFER_OVERRUN;
    uncomSize=LZSSUncompress(lzSize, outSize, Read, rData, Write, wData, obj);
    return (size==uncomSize)?uncomSize:-1;//size;//(size==uncomSize)?uncomSize:-1;
}
