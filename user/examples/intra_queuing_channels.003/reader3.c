...
void PartitionMain(void) {
 ...
 char msg2[38], msg3[38];
    ...
    if ((port2=XM_create_queuing_port("port2", 4, 64, XM_DESTINATION_PORT))<0) {
        xprintf("Error creating port2\n");
        return;
    }

    if ((port3=XM_create_queuing_port("port3", 4, 64, XM_DESTINATION_PORT))<0) {
        xprintf("Error creating port3\n");
        return;
    }

    if (XM_receive_queuing_message(port2, msg2, 38)<0) {
    ...
    if (XM_receive_queuing_message(port3, msg3, 38)<0) {
